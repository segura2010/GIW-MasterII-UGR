...
\begin{itemize}
	...
	\item \textbf{Grado Medio ($<k>$)}: 3.561
	\item \textbf{Diámetro ($d_{max}$)}: 2
	\item \textbf{Distancia Media ($<d>$)}: 1.013
	\item \textbf{Distancia Media para la red aleatoria equivalente ($<d_{aleatoria}>$)}: 
	\item \textbf{Coeficiente de Clustering Medio ($<C>$)}: 0.05
	\item \textbf{Coeficiente de Clustering Medio para la red aleatoria equivalente ($<C_{aleatoria}>$)}: 
\end{itemize}
