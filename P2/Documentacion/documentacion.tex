...
\begin{itemize}
	...
	\item \textbf{Distancia Media para la red aleatoria equivalente ($<d_{aleatoria}> = \frac{log(N)}{log(<k>)}$)}: 5.885
	\item \textbf{Coeficiente de Clustering Medio ($<C>$)}: 0.05
	\item \textbf{Coeficiente de Clustering Medio para la red aleatoria equivalente ($<C_{aleatoria}> = \frac{<k>}{N} $)}: 0.002
\end{itemize}
